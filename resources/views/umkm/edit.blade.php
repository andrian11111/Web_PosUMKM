...
<div class="flex h-screen bg-gray-100">
    ...
    <form action="{{ route('umkm.update', $umkm->id) }}" method="POST" enctype="multipart/form-data" class="bg-white p-16 rounded-lg shadow-lg w-full max-w-3xl" onsubmit="return confirmSave()">
            @csrf
...
<div class="space-y-5">
    ...
    <div class="flex">
        ...
        <input type="text" id="nama_umkm" name="nama_umkm" value="{{ $umkm->nama_umkm }}" class="input-text flex-1 rounded border-gray-300" required />
    </div>

    <!-- Alamat UMKM -->
    <div class="flex">
        <label for="alamat" class="text-lg font-medium w-1/3">Alamat:</label>
        <input type="text" id="alamat" name="alamat" value="{{ $umkm->alamat }}" class="input-text flex-1 rounded border-gray-300" required />
    </div>

    <!-- Telepon UMKM -->
    <div class="flex">
        <label for="nomor_telepon" class="text-lg font-medium w-1/3">Nomor Telepon:</label>
        <input type="text" id="nomor_telepon" name="nomor_telepon" value="{{ $umkm->nomor_telepon }}" class="input-text flex-1 rounded border-gray-300" required />
    </div>

    <!-- Harga Minimum -->
    <div class="flex">
        <label for="harga_min" class="text-lg font-medium w-1/3">Harga Minimum:</label>
        <input type="text" id="harga_min" name="harga_min" value="{{ $umkm->harga_min }}" class="input-text flex-1 rounded border-gray-300" required />
    </div>

    <!-- Harga Maximum -->
    <div class="flex">
        <label for="harga_max" class="text-lg font-medium w-1/3">Harga Maximum:</label>
        <input type="text" id="harga_max" name="harga_max" value="{{ $umkm->harga_max }}" class="input-text flex-1 rounded border-gray-300" required />
    </div>

    <!-- Deskripsi UMKM -->
<div class="flex">
    <label for="deskripsi" class="text-lg font-medium w-1/3">Deskripsi UMKM:</label>
    <textarea id="deskripsi" name="deskripsi" rows="4" class="input-text flex-1 rounded border-gray-300" required>{{ $umkm->deskripsi }}</textarea>
</div>

<!-- Gambar UMKM -->
<div class="flex">
    <label for="gambar_umkm" class="text-lg font-medium w-1/3">Gambar UMKM:</label>
    <input type="file" name="gambar_umkm" id="gambar_umkm" class="input-text flex-1 rounded border-gray-300" />
    @if ($umkm->gambar_umkm)
        <img src="{{ asset('storage/' . $umkm->gambar_umkm) }}" alt="Gambar UMKM" width="100" class="mt-2" />
    @endif
</div>
...
<div class="flex">
    ...
    <input type="file" name="gambar_menu" id="gambar_menu" class="input-text flex-1 rounded border-gray-300" />
    @if ($umkm->gambar_menu)
        <img src="{{ asset('storage/' . $umkm->gambar_menu) }}" alt="Gambar Menu" width="100" class="mt-2" />
    @endif
</div>
...
<div class="flex">
    ...
    <input type="file" name="gambar_unggulan" id="gambar_unggulan" class="input-text flex-1 rounded border-gray-300" />
    @if ($umkm->gambar_unggulan)
        <img src="{{ asset('storage/' . $umkm->gambar_unggulan) }}" alt="Gambar Produk Unggulan" width="100" class="mt-2" />
    @endif
</div>

    <!-- Kategori UMKM -->
    <div class="flex">
        <label for="kategori" value="{{ $umkm->kategori}}"  class="text-lg font-medium w-1/3 ">Kategori:</label>
